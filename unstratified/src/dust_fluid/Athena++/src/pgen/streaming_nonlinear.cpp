...
Real GasVel1Maximum(MeshBlock *pmb, int iout);
Real GasVel2Maximum(MeshBlock *pmb, int iout);
Real GasVel3Maximum(MeshBlock *pmb, int iout);

Real DustVel1Maximum(MeshBlock *pmb, int iout);
Real DustVel2Maximum(MeshBlock *pmb, int iout);
Real DustVel3Maximum(MeshBlock *pmb, int iout);
} // namespace
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  EnrollUserHistoryOutput(0, GasVel1Maximum, "GasVel1Max", UserHistoryOperation::max);
  EnrollUserHistoryOutput(1, GasVel2Maximum, "GasVel2Max", UserHistoryOperation::max);
  EnrollUserHistoryOutput(2, GasVel3Maximum, "GasVel3Max", UserHistoryOperation::max);

  EnrollUserHistoryOutput(3, DustVel1Maximum, "DustVel1Max", UserHistoryOperation::max);
  EnrollUserHistoryOutput(4, DustVel2Maximum, "DustVel2Max", UserHistoryOperation::max);
  EnrollUserHistoryOutput(5, DustVel3Maximum, "DustVel3Max", UserHistoryOperation::max);

  EnrollUserHistoryOutput(6, DustFluidsRatioMaximum, "RatioMax", UserHistoryOperation::max);
...
Real GasVel1Maximum(MeshBlock *pmb, int iout) {
  ...
  Real gas_vel1_max = 0.0;
  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
      for (int i=is; i<=ie; i++) {
        Real &gas_vel1 = w(IM1, k, j, i);
        gas_vel1_max   = std::max(gas_vel1_max, std::abs(gas_vel1));
      }
    }
  }
  return gas_vel1_max;
...
Real GasVel2Maximum(MeshBlock *pmb, int iout) {
  ...
  Real gas_vel2_max = 0.0;
  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
      for (int i=is; i<=ie; i++) {
        Real &gas_vel2 = w(IM2, k, j, i);
        gas_vel2_max   = std::max(gas_vel2_max, std::abs(gas_vel2));
      }
    }
  }
  return gas_vel2_max;
...
Real GasVel3Maximum(MeshBlock *pmb, int iout) {
  ...
  Real gas_vel3_max = 0.0;
  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
      for (int i=is; i<=ie; i++) {
        Real &gas_vel3 = w(IM3, k, j, i);
        gas_vel3_max   = std::max(gas_vel3_max, std::abs(gas_vel3));
      }
    }
  }
  return gas_vel3_max;
...
Real DustVel1Maximum(MeshBlock *pmb, int iout) {
  ...
  Real dust_vel1_max = 0.0;
  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
      for (int i=is; i<=ie; i++) {
        Real &dust_vel1 = df_w(IM1, k, j, i);
        dust_vel1_max   = std::max(dust_vel1_max, std::abs(dust_vel1));
      }
    }
  }
  return dust_vel1_max;
}
...
Real DustVel2Maximum(MeshBlock *pmb, int iout) {
  ...
  Real dust_vel2_max = 0.0;
  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
      for (int i=is; i<=ie; i++) {
        Real &dust_vel2 = df_w(IM2, k, j, i);
        dust_vel2_max   = std::max(dust_vel2_max, std::abs(dust_vel2));
      }
    }
  }
  return dust_vel2_max;
}
...
Real DustVel3Maximum(MeshBlock *pmb, int iout) {
  ...
  Real dust_vel3_max = 0.0;
  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
      for (int i=is; i<=ie; i++) {
        Real &dust_vel3 = df_w(IM3, k, j, i);
        dust_vel3_max   = std::max(dust_vel3_max, std::abs(dust_vel3));
      }
    }
  }
  return dust_vel3_max;
}
}
