 ...
 * hst_rhop_dis()    - particle density dispersion. Dumped in .hst file because 
 *                     it uses grid-based density; JL Nov. 2023th 
 *============================================================================*/
...
/*=========================== PUBLIC FUNCTIONS =================================
 *============================================================================*/
static Real hst_rhop(const GridS *pG,const int i,const int j,const int k); //Average of particle density, JL, Nov.2023
static Real hst_rhopsq(const GridS *pG,const int i,const int j,const int k); //Average of particle density squared; JL, Nov.2023
/*----------------------------------------------------------------------------*/
/* problem:   */
...
#ifdef MPI_PARALLEL
  long int iseed = myID_Comm_world;
#else
...
//                                        +0.5*amp*sin(2.0*kx*x1p))/kx;
  ...
  //dump_history_enroll(hst_rhopsq, "<rhopsq>"); // JL, Nov. 2023 
  //dump_history_enroll(hst_rhop, "<rhop>"); // JL, Nov. 2023 
...
/*----------------------------------------------------------------------------*/
/*! \fn static Real hst_rhop(const GridS *pG, const int i, const int j,
 *                          const int k)
 *  \brief Computes average of particle density (grid-based)
 */
 static Real hst_rhop(const GridS *pG, const int i, const int j, const int k)
 {
   Real x1,x2,x3, rhop;
   cc_pos(pG,i,j,k,&x1,&x2,&x3);
   
   rhop   = expr_dpar(pG,i,j,k);
...
/*! \fn static Real hst_rhop(const GridS *pG, const int i, const int j,
 *                          const int k)
 *  \brief Computes average of particle density squared (grid-based)
 */
 static Real hst_rhopsq(const GridS *pG, const int i, const int j, const int k)
 {
   Real x1,x2,x3, rhopsq;
   cc_pos(pG,i,j,k,&x1,&x2,&x3);
   
   rhopsq   = SQR(expr_dpar(pG,i,j,k));
